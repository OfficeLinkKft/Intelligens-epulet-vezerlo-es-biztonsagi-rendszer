...
if ($_SERVER["REQUEST_METHOD"] == "POST") { # posted datas?
		...
		while($row = mysqli_fetch_array($result))
		{
			...
			while($row[$Nd]) {
				...
				if ( isset($_POST[$row[$Nd]]) ) {
					$act_inv_outSys = $_POST[$row[$Nd]] ;

					echo "<h1>act_inv_outSys: ".$act_inv_outSys."</h1>"; ## check activity
				}
				$Nd++;			
			}
		}
	...
	echo "<h2><b>act_inv / PN / SN / self: </b>".$act_inv_outSys." / ".$PN." / ".$SN." / ".$self."</h2>";    # for test
	...
	if ($self=="submit") {
		...
		if ($act_inv_outSys=="beépítés") {# 
			...
			if ( $date=="" ) { # missing
				...
				mysqli_query($conn,"INSERT INTO inv_log6 (PN,SN,systempart,location,id,name,date,operator,inv_act,status,set_for,port,ip,warranty,last,note) 
					VALUES ('$PN','$SN','$location','$location','$id','$name','$date','$_SESSION[nick]','$act_inv_outSys','ok','$set_for','$port','$ip','$warranty','1','$note')");
		...
		# activity -> check datas and write to table new line to inv_log6
		if ($act_inv_outSys=="hiba") {# 

			#  ***   check datas
			if ( $date=="" ) { # missing
				# set button message
				$end_message = "data MISS! - fill up and try again ! >> &nbsp;&nbsp;"; 
				...
				#  ***   datas to database   ***
				mysqli_query($conn,"INSERT INTO inv_log6 (PN,SN,systempart,location,id,name,date,operator,inv_act,status,set_for,port,ip,warranty,last,note) 
					VALUES ('$PN','$SN','$systempart','$location','$id','$name','$date','$_SESSION[nick]','$act_inv_outSys','hiba','$set_for','$port','$ip','$warranty','1','$note')");
		...
		if ($act_inv_outSys=="javítás") {# 
			...
			if ( $date=="" ) { # missing
				...
				#  ***   datas to database   ***
				mysqli_query($conn,"INSERT INTO inv_log6 (PN,SN,systempart,location,id,name,date,operator,inv_act,status,set_for,port,ip,warranty,last,note) 
					VALUES ('$PN','$SN','$systempart','$location','$id','$name','$date','$_SESSION[nick]','$act_inv_outSys','ok','$set_for','$port','$ip','$warranty','1','$note')");
		...
		if ($act_inv_outSys=="selejt") {# 
			...
			if ( $date=="" ) { # missing
				...
				#  ***   new line with new datas to database   ***
				mysqli_query($conn,"INSERT INTO inv_log6 (PN,SN,systempart,location,id,name,date,operator,inv_act,status,set_for,port,ip,warranty,last,note) 
					VALUES ('$PN','$SN','no','műhely','$id','$name','$date','$_SESSION[nick]','$act_inv_outSys','selejt','$set_for','$port','$ip','$warranty','1','$note')");
...
<div class="header">
	...
	<div class="hname">Elem művelet - outSys</div>
...
<table class=table>
  ...
  <tr> 
    <td align="center" style=<?php echo $bgc;?>;><b>
	  	<?php echo "(".$N.") ".$TOP." - ".$PN." / ".$SN." - ".$act_inv_outSys." / státusz: "."$status"; ?>
    </b></td>
  </tr>

  <tr> 
    <td valign="top" style="background-color:<?=$outSys_color?>">

    <form method="post" action="
		<?php
			$self = 'submit';
			echo 'act_inv_outSys.php?self='.$self.'&PN='.$PN.'&SN='.$SN; 
		?>
	"> 
       ...
       <tr><td>
		...
		?>
			...
			<tr><td>
				...
				<input list="location" name="location_i" value="<?php echo $location; ?>">
				<span class="error"> * nem beépítés</span>
			</td></tr>
		...
		<?php 
		## ***  in case of beépítés / implement  activity   ***
		if ($act_inv_outSys=="beépítés") {
		?>
			...
			<tr><td>
				...
				<input list="location" name="location_i" value="<?php echo $location; ?>">
				<span class="error"> * ide kerül beépítésre</span>
			</td></tr>
		...
		<?php 
		} ## ***  in case of implement (bebeépítés) activity   ***
		?>
		...
		if ( ($act_inv_outSys=="hiba") OR ($act_inv_outSys=="javítás") OR ($act_inv_outSys=="selejt") ) {
		?>
		...
		<tr><td colspan=2 style="border:0px;">
			...
			<?php
				...
				echo '<input style="width:70px;margin:10px;padding:5px;font-weight:bold;" type="submit" name="'.$act_inv_outSys.'" value="'.$act_inv_outSys.'">';
			?>
