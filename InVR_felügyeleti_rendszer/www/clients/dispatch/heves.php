...
<html>
<head>
<title>Webkamer�k - Heves megye</title>
<meta http-equiv="refresh" content="55;url=heves.php">
...
//	$n=$_SESSION['nick'];
	$n='heves';
		...
		elseif($n =='heves') {
				$result = mysql_query("SELECT * FROM clients WHERE owner LIKE 'MK' AND county LIKE 'Heves, K06' AND OFF!='OFF' ORDER BY id");
				$result2 = mysql_query("SELECT avg(avail28days) FROM clients WHERE owner LIKE 'MK' AND county LIKE 'Heves, K06' AND OFF!='OFF'"); }
...
<div class="box">
	...
	<div class="header">
		...
		<div class="sw1">
			<a href="../clients_heves.php" target="_blank">kliensek</a>
		</div>
		<div class="sw1">
			<a href="../map_heves.php" target="_blank" title="t�rk�p"><img src="../images/map_tmb.png" width="27" height="18"></a>
		</div>
		<! ***** name  *****>
			...
			// ***** pix  *****
				if($row[cam1]!="no")
				{
					...
					echo "<div class='pix'>";
						...
						echo "<div class='thumbnail'>
							<a href=\"".substr($row[web],strpos($row[web],'/')). "/webcam1.jpg\" width=\"600\" height=\"400\" rel=\"lytebox[cam]\" title='Hely: ".$row[address]." - Kamerair�ny: ".$row[cam1]." - Leveg� h�m�rs�klet: ".$data."'><img src=\"".substr($row[web],strpos($row[web],'/'))."/webcam1.jpg\" width=".$pw." height=".$ph."></a>
						</div>";
					echo "</div>";
				}
				if($row[cam2]!="no")
				{
					...
					echo "<div class='pix'>";
						...
						echo "<div class='thumbnail'>
							<a href=\"".substr($row[web],strpos($row[web],'/')). "/webcam2.jpg\" width=\"600\" height=\"400\" rel=\"lytebox[cam]\" title='Hely: ".$row[address]." - Kamerair�ny: ".$row[cam2]." - Leveg� h�m�rs�klet: ".$data."'><img src=\"".substr($row[web],strpos($row[web],'/'))."/webcam2.jpg\" width=".$pw." height=".$ph."></a>
						</div>";
					echo "</div>";
				}
				if($row[cam3]!="no")
